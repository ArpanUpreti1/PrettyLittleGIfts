...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setResponseMessage({ text: 'Message sent successfully!', type: 'success' });
        setFormData({ name: '', email: '', message: '' });
      } else {
        setResponseMessage({ text: 'Failed to send message.', type: 'error' });
      }
    } catch (error) {
      setResponseMessage({ text: 'An error occurred. Please try again later.', type: 'error' });
    }
  };
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input type="text" id="name" value={formData.name} onChange={handleChange} className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-[#B8860B] focus:border-[#B8860B]" required />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-bold text-gray-700">Email</label>
                <input type="email" id="email" value={formData.email} onChange={handleChange} className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-[#B8860B] focus:border-[#B8860B]" required />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-bold text-gray-700">Message</label>
                <textarea id="message" rows="5" value={formData.message} onChange={handleChange} className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-[#B8860B] focus:border-[#B8860B]" required></textarea>
              </div>
              <motion.button
                type="submit"
            ...
            {responseMessage && (
              <div className={`mt-4 text-center p-4 rounded-lg ${responseMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
